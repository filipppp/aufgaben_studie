const input2 = `L777FJF77F-77-F-7F7.FF.J7.-J--J7F|7F.FJ.L-|F|-F|FJ7-J7F-JFF-7.L7FL7J-FF|7--7.L--|77.F-J77.-7FJ7F7FF--.FFJ7-7-F|.FL-L-F7.F|-FF7F|-7-|7-.FF-7.
...
const distanceMatrix = matrix.map(row => row.map(() => -1));
distanceMatrix[sCoord.row][sCoord.col] = 0;

function doesAdjacentConnectToDirection(direction: Direction, adjacentSymbol: string) {
    switch (direction) {
        case Direction.TOP:
            return "|7FS".includes(adjacentSymbol)
        case Direction.BOTTOM:
            return "|LJS".includes(adjacentSymbol)
        case Direction.LEFT:
            return "-LFS".includes(adjacentSymbol)
        case Direction.RIGHT:
            return "-J7S".includes(adjacentSymbol)
    }
}

function directionsToCheck(symbol: string) {
    switch (symbol) {
        case "F":
...
function getNextConnectingPoint(coord: Coord, previousPoint?: Coord) {
    let {row, col} = coord;
    const symbol = matrix[row][col];
    const directions = directionsToCheck(symbol);

    let coords: Coord[] = [];
    for (let direction of directions) {
        switch (direction) {
            case Direction.TOP:
                if (row - 1 >= 0 && doesAdjacentConnectToDirection(direction, matrix[row - 1][col])) {
                    coords.push({row: row - 1, col})
                }
                break;
            case Direction.BOTTOM:
                if (row + 1 < matrix.length && doesAdjacentConnectToDirection(direction, matrix[row + 1][col])) {
                    coords.push({row: row + 1, col})
                }
                break;
            case Direction.LEFT:
                if (col - 1 >= 0 && doesAdjacentConnectToDirection(direction, matrix[row][col - 1])) {
                    coords.push({row, col: col - 1})
                }
                break;
            case Direction.RIGHT:
                if (col + 1 < matrix[row].length && doesAdjacentConnectToDirection(direction, matrix[row][col + 1])) {
                    coords.push({row, col: col + 1})
                }
                break;
        }
    ...
    if (previousPoint) {
        return coords.filter(c => previousPoint.row !== c.row || previousPoint.col !== c.col);
    }
    return coords;
}

let nextPoints = getNextConnectingPoint(sCoord);
let prevPoints = [sCoord, sCoord];
while (true) {
    let newNextPoints = [];
    for (let i = 0; i < nextPoints.length; i++) {
        distanceMatrix[nextPoints[i].row][nextPoints[i].col] = distanceMatrix[prevPoints[i].row][prevPoints[i].col] + 1;
        newNextPoints.push(getNextConnectingPoint(nextPoints[i], prevPoints[i])[0])
    }
    prevPoints = nextPoints;
    nextPoints = newNextPoints;
    console.log("nextPoints: ",nextPoints)
    console.log("prevPoints: ", prevPoints)
    if (prevPoints[0].row === prevPoints[1].row && prevPoints[0].col === prevPoints[1].col) {
        break;
    }
}

console.log(distanceMatrix.map(row => row.map(num => num === -1 ? "." : num).join("")).join("\n"))
console.log(Math.max(...distanceMatrix.map(row => Math.max(...row))))
